...
class SNSCoordImpl final : public SNSCoord::Service {
  ...
  Status Gucci(ServerContext* context, const HrtBt* request, HrtBt* response){
    ...
    try {
      ...
      if (isMaster) cif->mbeats++;
      else cif->sbeats++;
    } 
    catch (const std::out_of_range& oor){
      std::cerr << "A bad heartbeat was recieved, with id: " << id << std::endl;
      if (isMaster) std::cerr << "Claiming to be Master." << std::endl;
      else std::cerr << "Claiming to be Slave." << std::endl;
      return Status::CANCELLED;
    }
...
void checkCluster(struct clustinfo* cif){
  ...
  for(;;){
    ...
    if (!cif->master_live && cif->mbeats > 0) cif->master_live = true; //ressurection
    if(mbeats - cif->mbeats > 2){ //if we miss 2 there's a failure
      //He's dead Jim...
      mbeats = 0;
      cif->mbeats = 0;
      cif->master_live = false;
    }
    else if (cif->master_live){ //If we're alive, we need to add a beat
      mbeats++;
    }
    if (!cif->slave_live && cif->sbeats > 0) cif->slave_live = true;
